...
## Project status and intended direction

**Implemented in v0.2:** a declaration parser, static validator, JSON-manifest compiler, and command-line interface for checking and compiling ABHL files. The compiler describes architectural relationships; it does not execute the moving handoff loop.

**Test coverage:** four unit tests cover the valid CI declaration example and rejection of cross-scope Ground mutation, authority transfer through a handoff, and privilege growth through widening. These verify selected behavior, not the complete A/B method.

**Intended runtime (not yet implemented):** A/B resolution → typed handoff → receiver-owned branch work → returned result → re-grounding → renewed A/B. A temporary widened scope must complete its own re-grounding and renewed A/B before reconnecting.

**Intended housekeeping (not yet automated):** when A/B work is already moving, resolve bookkeeping issues arising inside that work before they accumulate downstream. This includes tracking the provenance and status of temporarily borrowed terms, notation, models, examples, or structures so inherited, adapted, retained, and discarded material stays distinguishable. This is part of active A/B work, **not** a requirement to A/B every ordinary management task.

**Open:** full runtime execution, branch-specific evidence semantics, operational re-grounding, legitimate OPEN/PINNED outcomes in a running system, and comparative validation. `docs/VALIDATION_PROTOCOL.md` defines an evaluation protocol; it does not report completed comparative trials.

v0.2 can remain a stable, bounded experimental prototype; further development is optional.
